#!/bin/bash
# Install JARVIS OS components on a Debian/Ubuntu-based system.
# No kernel install — JARVIS runs on the stock kernel.
#
# Required env vars:
#   JARVIS_MODEL  — e.g. "qwen3:4b"
#   JARVIS_ENV    — "wsl" or "linux-debian" (set by task-overlay-detect.sh)
...
JARVIS_ENV="${JARVIS_ENV:-linux-debian}"
_is_wsl=false
[[ "${JARVIS_ENV}" == "wsl" ]] && _is_wsl=true

trap 'echo "JARVIS Debian overlay failed at line $LINENO: $BASH_COMMAND" >&2' ERR

# ── Ensure apt deps ────────────────────────────────────────────────────────
info "Updating apt package index..."
apt-get update -q || warn "apt-get update had issues — continuing"

info "Installing required host tools..."
DEBIAN_FRONTEND=noninteractive apt-get install -y \
    dialog curl wget git python3 python3-pip python3-venv python3-dev \
    gcc make pkg-config portaudio19-dev unzip \
    || warn "Some host tool packages failed"

# ── Base utilities ────────────────────────────────────────────────────────
info "Installing base utilities..."
DEBIAN_FRONTEND=noninteractive apt-get install -y \
    sudo less nano vim wget curl git openssh-client man-db \
    unzip zip rsync tzdata bash-completion lsof htop \
    || warn "Some base packages failed"
ok "Base utilities installed"

# ── KDE Plasma (skip in WSL) ──────────────────────────────────────────────
if ! $_is_wsl; then
    info "Installing KDE Plasma Wayland..."
    DEBIAN_FRONTEND=noninteractive apt-get install -y \
        kde-plasma-desktop sddm \
        plasma-nm plasma-pa kwin-wayland \
    ...
    DEBIAN_FRONTEND=noninteractive apt-get install -y \
        mesa-vulkan-drivers libva-drm2 libva-x11-2 \
        || warn "Some GPU packages failed"
    ok "GPU drivers installed"
else
    info "WSL detected — skipping desktop environment, display manager, GPU drivers."
fi
...
DEBIAN_FRONTEND=noninteractive apt-get install -y \
    ...
    gcc make pkg-config portaudio19-dev \
...
if systemctl --version >/dev/null 2>&1; then
    ...
    warn "systemd not available — services not enabled (WSL without systemd?)"
    warn "Start JARVIS manually: sudo /usr/bin/jarvis-daemon"
fi
...
echo ""
echo -e "${GREEN}${BOLD}  JARVIS OS components installed (Debian/Ubuntu).${NC}"
if $_is_wsl; then
    echo ""
    echo "  WSL note: if systemd is not enabled in WSL, start JARVIS manually:"
    echo "    sudo /usr/bin/jarvis-daemon &"
